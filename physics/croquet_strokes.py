...
class CroquetStrokeCalculator:
    ...
    STROKE_RATIOS = {
        StrokeType.STOP_SHOT: 0.15,    # Striker moves 15% of croqueted distance
    ...
    def calculate_stroke(
        ...
        if stroke_type == StrokeType.TAKE_OFF:
            ...
            # Increase multiplier to allow longer placements
            croqueted_speed = power / (1 + ratio) * 1.8  # Increased from 1.2 for longer shots
    ...
    def _calculate_split(
        ...
        # But we need enough power for long pioneer placements (10+ yards)
        split_factor = math.cos(half_angle)  # Reduces power as angle increases
        ball_power = power * split_factor * 0.95  # Only slight reduction for split mechanics
    ...
    def _power_for_distance(self, distance: float) -> float:
        ...
        # Allow higher power for croquet strokes - up to 18 yards/s for long placements
        max_croquet_power = config.MAX_SHOT_POWER * 1.5
        return min(velocity * 1.4, max_croquet_power)
